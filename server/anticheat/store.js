...
function createJsonStore(file) {
  let data = { bans: {}, kicks: {}, chatMutes: {} };
  let timer = null;
  try {
    fs.mkdirSync(path.dirname(file), { recursive: true });
    if (fs.existsSync(file)) data = Object.assign({ bans: {}, kicks: {}, chatMutes: {} }, JSON.parse(fs.readFileSync(file, 'utf8')));
  } catch (e) {
  ...
  return {
    ...
    getChatMute(ident) {
      const m = data.chatMutes[ident];
      if (!m) return null;
      if (m.until <= Date.now()) { delete data.chatMutes[ident]; save(); return null; }
      return m;
    },
    setChatMute(ident, mute) { data.chatMutes[ident] = mute; save(); },
    // 记录一次踢出，返回窗口期内累计次数
...
function createMemoryStore() {
  const data = { bans: {}, kicks: {}, chatMutes: {} };
  return {
    getBan(ident) {
      const b = data.bans[ident];
    ...
    setChatMute(ident, mute) { data.chatMutes[ident] = mute; },
    addKick(ident, windowMs) {
      const t = Date.now();
      const arr = (data.kicks[ident] || []).filter(ts => t - ts < windowMs);
      arr.push(t);
