...
class AntiCheat {
  ...
  // 公屏禁言（仅封 IP，不影响进房与游戏）
  isChatMuted(ip) {
    if (!this.opts.store || !ip) return null;
    const key = 'ip:' + normalizeIp(ip);
    if (key === 'ip:unknown') return null;
    return this.opts.store.getChatMute(key);
  }
  registerChatMute(ip, minutes, reason) {
    if (!this.opts.store || !ip) return;
    const key = 'ip:' + normalizeIp(ip);
    if (key === 'ip:unknown') return;
    this.opts.store.setChatMute(key, { until: now() + minutes * 60000, reason, at: now() });
  }
