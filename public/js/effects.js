...
G.fx = (function () {
  ...
  // ---------- 电磁炮光柱残影（半径与 server beamRadius 对齐：外圈 halo=0.58） ----------
  const BEAM_MAX = 10;
  const BEAM_LIFE = 0.75;
  const BEAM_R = { core: 0.09, glow: 0.32, halo: 0.58, aura: 0.78 };
  const beams = [];
  function railBeam(o, e) {
    let b = beams.find(q => !q.alive);
    if (!b) return;
    const a = V(o[0], o[1], o[2]), end = V(e[0], e[1], e[2]);
    const len = a.distanceTo(end);
    if (len < 0.2) return;
    const dir = end.clone().sub(a).normalize();
    const mid = a.clone().add(end).multiplyScalar(0.5);
    b.alive = true; b.t = 0; b.len = len;
    b.group.visible = true;
    b.group.position.copy(mid);
    b.group.quaternion.setFromUnitVectors(V(0, 1, 0), dir);
    b.core.material.color.set(0xe8ffff);
    b.glow.material.color.set(0x35e0ff);
    b.halo.material.color.set(0x8f6bff);
    b.aura.material.color.set(0x4a9fff);
    b.core.scale.set(BEAM_R.core, len, BEAM_R.core);
    b.glow.scale.set(BEAM_R.glow, len, BEAM_R.glow);
    b.halo.scale.set(BEAM_R.halo, len, BEAM_R.halo);
    b.aura.scale.set(BEAM_R.aura, len, BEAM_R.aura);
    b._rx = BEAM_R.core; b._rg = BEAM_R.glow; b._rh = BEAM_R.halo; b._ra = BEAM_R.aura;
    b.core.material.opacity = 1;
    b.glow.material.opacity = 0.88;
    b.halo.material.opacity = 0.55;
    b.aura.material.opacity = 0.28;
    b.light.position.copy(mid);
    b.light.intensity = 8;
    b.light.distance = Math.min(48, len * 0.45);
    b.light.color.set(0x6ef0ff);
    const sparks = Math.min(40, Math.floor(len / 1.6));
    for (let i = 0; i < sparks; i++) {
      const p = a.clone().lerp(end, Math.random());
      const spread = BEAM_R.halo * (0.4 + Math.random() * 0.9);
      spawnP(p, V((Math.random() - 0.5) * spread, (Math.random() - 0.5) * spread, (Math.random() - 0.5) * spread), i % 3 ? '#9f7bff' : '#7df9ff', 0.28 + Math.random() * 0.22, 0.32 + Math.random() * 0.4);
    }
    burst(a, 18, '#d8ffff', 8, 0.28, 0.48);
    burst(end, 16, '#5ce8ff', 7, 0.26, 0.42);
    shakeAmt = Math.min(1.2, shakeAmt + 0.42);
  }

  // ---------- 充能步枪橙色光束（充能细线；崩击残影半径对齐电磁炮 BEAM_R） ----------
  const CHARGE_R = { core: 0.018, glow: 0.05, halo: 0.09 };
  const chargeBeams = new Map(); // id -> mesh pack
  let chargePoolScene = null;
  function ensureChargeBeam(id) {
    let b = chargeBeams.get(id);
    if (b) return b;
    if (!chargePoolScene) return null;
    const geo = new T.CylinderGeometry(1, 1, 1, 10, 1, true);
    const core = new T.Mesh(geo, new T.MeshBasicMaterial({ color: 0xffe0a0, transparent: true, opacity: 1, blending: T.AdditiveBlending, depthWrite: false }));
    const glow = new T.Mesh(geo, new T.MeshBasicMaterial({ color: 0xff7a1a, transparent: true, opacity: 0.85, blending: T.AdditiveBlending, depthWrite: false }));
    const halo = new T.Mesh(geo, new T.MeshBasicMaterial({ color: 0xff4500, transparent: true, opacity: 0.4, blending: T.AdditiveBlending, depthWrite: false }));
    const group = new T.Group();
    group.add(core, glow, halo);
    group.visible = false;
    const light = new T.PointLight(0xff7a1a, 0, 24, 2);
    chargePoolScene.add(group); chargePoolScene.add(light);
    b = { group, core, glow, halo, light, alive: false, pulse: 0 };
    chargeBeams.set(id, b);
    return b;
  }
  function setChargeBeam(id, o, e) {
    const b = ensureChargeBeam(id);
    if (!b) return;
    const a = V(o[0], o[1], o[2]), end = V(e[0], e[1], e[2]);
    const len = a.distanceTo(end);
    if (len < 0.15) return;
    ...
    b.group.position.copy(mid);
    b.group.quaternion.setFromUnitVectors(V(0, 1, 0), dir);
    const pulse = 1 + Math.sin(b.pulse * 40) * 0.08;
    b.core.scale.set(CHARGE_R.core * pulse, len, CHARGE_R.core * pulse);
    b.glow.scale.set(CHARGE_R.glow * pulse, len, CHARGE_R.glow * pulse);
    b.halo.scale.set(CHARGE_R.halo * pulse, len, CHARGE_R.halo * pulse);
    b.core.material.opacity = 0.95;
    b.glow.material.opacity = 0.8;
    b.halo.material.opacity = 0.42;
    b.light.position.copy(mid);
    b.light.intensity = 3.2;
    b.light.distance = Math.min(28, len * 0.35);
  }
  function clearChargeBeam(id) {
    const b = chargeBeams.get(id);
    if (!b) return;
    b.alive = false;
    b.group.visible = false;
    b.light.intensity = 0;
  }
  function chargeBangFx(o, e) {
    const a = V(o[0], o[1], o[2]), end = V(e[0], e[1], e[2]);
    const len = a.distanceTo(end);
    if (len > 0.2) {
      const dir = end.clone().sub(a).normalize();
      const mid = a.clone().add(end).multiplyScalar(0.5);
      // 借用电磁炮光束池：崩击光柱半径与 BEAM_R（电磁炮）一致
      let b = beams.find(q => !q.alive);
      if (b) {
        b.alive = true; b.t = 0; b.len = len;
        b.group.visible = true;
        ...
        b.aura.scale.set(BEAM_R.aura, len, BEAM_R.aura);
        b._rx = BEAM_R.core; b._rg = BEAM_R.glow; b._rh = BEAM_R.halo; b._ra = BEAM_R.aura;
        b.core.material.color.set(0xfff0c8);
        b.glow.material.color.set(0xff7a1a);
        b.halo.material.color.set(0xff4500);
        b.aura.material.color.set(0xff9a3c);
        b.core.material.opacity = 1;
        b.glow.material.opacity = 0.88;
        b.halo.material.opacity = 0.55;
        b.aura.material.opacity = 0.28;
        ...
        b.light.intensity = 9;
        b.light.distance = Math.min(48, len * 0.45);
        b.light.color.set(0xff7a1a);
        const sparks = Math.min(40, Math.floor(len / 1.6));
        for (let i = 0; i < sparks; i++) {
          const p = a.clone().lerp(end, Math.random());
          const spread = BEAM_R.halo * (0.4 + Math.random() * 0.9);
          spawnP(p, V((Math.random() - 0.5) * spread, (Math.random() - 0.5) * spread, (Math.random() - 0.5) * spread), i % 3 ? '#ff6a1a' : '#ffc878', 0.28 + Math.random() * 0.22, 0.32 + Math.random() * 0.4);
        }
      }
    }
    burst(a, 18, '#ffb060', 8, 0.28, 0.48);
    burst(end, 16, '#ff6a1a', 7, 0.26, 0.42);
    impactSpark([end.x, end.y, end.z], '#ff9a3c');
    shakeAmt = Math.min(1.2, shakeAmt + 0.42);
  }
  ...
  function damageText(pos, text, color, big) {
    ...
    if (!d && dtexts.length) {
      d = dtexts[0];
      for (const q of dtexts) if (q.t > d.t) d = q;
    }
    if (!d) return;
    d.alive = true; d.t = 0;
  ...
  function init(sc) {
    ...
    const beamGeo = new T.CylinderGeometry(1, 1, 1, 12, 1, true);
    for (let i = 0; i < BEAM_MAX; i++) {
      const core = new T.Mesh(beamGeo, new T.MeshBasicMaterial({ color: 0xe8ffff, transparent: true, opacity: 1, blending: T.AdditiveBlending, depthWrite: false }));
      const glow = new T.Mesh(beamGeo, new T.MeshBasicMaterial({ color: 0x35e0ff, transparent: true, opacity: 0.88, blending: T.AdditiveBlending, depthWrite: false }));
      const halo = new T.Mesh(beamGeo, new T.MeshBasicMaterial({ color: 0x8f6bff, transparent: true, opacity: 0.55, blending: T.AdditiveBlending, depthWrite: false }));
      const aura = new T.Mesh(beamGeo, new T.MeshBasicMaterial({ color: 0x4a9fff, transparent: true, opacity: 0.28, blending: T.AdditiveBlending, depthWrite: false }));
      core.scale.set(BEAM_R.core, 1, BEAM_R.core);
      glow.scale.set(BEAM_R.glow, 1, BEAM_R.glow);
      halo.scale.set(BEAM_R.halo, 1, BEAM_R.halo);
      aura.scale.set(BEAM_R.aura, 1, BEAM_R.aura);
      const group = new T.Group();
      group.add(aura, halo, glow, core);
      group.visible = false;
      scene.add(group);
      const light = new T.PointLight(0x6ef0ff, 0, 48);
      group.add(light);
      beams.push({ alive: false, group, core, glow, halo, aura, light, t: 0, len: 1 });
    }
  ...
  function update(dt) {
    ...
    for (const b of beams) {
      if (!b.alive) continue;
      b.t += dt;
      if (b.t >= BEAM_LIFE) { b.alive = false; b.group.visible = false; b.light.intensity = 0; continue; }
      const k = b.t / BEAM_LIFE;
      const fade = Math.pow(1 - k, 1.6);
      const pulse = 1 + Math.sin(b.t * 48) * 0.06 * fade;
      b.core.material.opacity = fade;
      b.glow.material.opacity = fade * 0.82;
      b.halo.material.opacity = fade * 0.5;
      b.aura.material.opacity = fade * 0.26;
      const rx = b._rx || BEAM_R.core;
      const rg = b._rg || BEAM_R.glow;
      const rh = b._rh || BEAM_R.halo;
      const ra = b._ra || BEAM_R.aura;
      b.core.scale.x = b.core.scale.z = rx * pulse;
      b.glow.scale.x = b.glow.scale.z = rg * pulse;
      b.halo.scale.x = b.halo.scale.z = rh * (1 + (1 - fade) * 0.28);
      b.aura.scale.x = b.aura.scale.z = ra * (1 + (1 - fade) * 0.45);
      b.light.intensity = fade * 7.5;
    }
    for (const b of chargeBeams.values()) {
      if (!b.alive) continue;
      b.pulse += dt;
    }
  ...
  function envyRing(pos, r, ms) {
    telegraph(pos, r, ms);
    burst(pos, 14, '#ff8fb8', 3.2, 0.12, 0.7);
    burst(pos, 10, '#2a1020', 2.2, 0.16, 0.9);
  }

  function fateWoundFx(pos) {
    burst(pos, 12, '#ff8fb8', 2.8, 0.1, 0.55);
    sparkle(pos, '#ffc0d8');
  }

  function ckptSaveFx(pos) {
    respawnBeam(pos);
    sparkle([pos[0], (pos[1] || 0) + 1.2, pos[2]], '#ff8fb8');
    burst([pos[0], (pos[1] || 0) + 0.4, pos[2]], 16, '#ff8fb8', 2.4, 0.1, 0.8);
  }

  function rbDeathFx(from, to) {
    sparkle(from, '#2a1020');
    sparkle(to, '#ff8fb8');
    burst(from, 18, '#ff8fb8', 4, 0.12, 0.65);
    burst(to, 18, '#ff8fb8', 3.2, 0.12, 0.7);
    shakeAmt = Math.min(1.2, shakeAmt + 0.35);
  }

  function voidRiftOpen(rifts, center) {
    const list = Array.isArray(rifts) && rifts.length ? rifts : (center ? [center] : []);
    for (const p of list) {
      if (!p) continue;
      burst(p, 12, '#9b5cff', 2.8, 0.11, 0.6);
      burst(p, 10, '#1a0a28', 1.6, 0.16, 0.85);
      sparkle(p, '#c49bff');
    }
    if (list.length) shakeAmt = Math.min(1.2, shakeAmt + 0.18);
  }

  return {
    init, update, tracer, railBeam, setChargeBeam, clearChargeBeam, chargeBangFx, muzzle, impact, impactSpark, blood, explosion, telegraph, dustPuff, slam, die, respawnBeam, sparkle, roarWave, damageText,
    flashPop, smokeCloud, envyRing, fateWoundFx, ckptSaveFx, rbDeathFx, voidRiftOpen,
