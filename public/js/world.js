...
G.world = (function () {
  ...
  function rampGeometry(r) {
    ...
    const w = r.w / 2;
    const A = rampCoord(r, low, y0, -w);
    const B = rampCoord(r, low, y0, w);
    const C = rampCoord(r, high, r.h, w);
    const D = rampCoord(r, high, r.h, -w);
    const E = rampCoord(r, cap, y0, -w);
    const F = rampCoord(r, cap, y0, w);
  ...
  function rampHeightAt(pos, pad) {
    ...
    for (const r of ramps) {
      ...
      const y0 = r.y0 || 0;
      const h = y0 + Math.max(0, Math.min(1, t)) * (r.h - y0);
