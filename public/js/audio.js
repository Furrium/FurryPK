...
G.audio = (function () {
  ...
  return {
    ...
    shot(wp) {
      ...
      else if (wp === 'shotgun') { burst(0.22, 900, 0.75, 0.22); tone('sawtooth', 180, 55, 0.22, 0.32); burst(0.15, 2200, 0.35, 0.08, 'highpass'); }
      else if (wp === 'sniper') { burst(0.35, 1400, 0.8, 0.3); tone('sawtooth', 190, 40, 0.3, 0.28); burst(0.4, 300, 0.4, 0.4); }
      else if (wp === 'charge') { /* 充能步枪：开火音在 chargeZap / chargeBang 分段播 */ }
      else if (wp === 'railgun') { burst(0.06, 3800, 0.35, 0.05, 'highpass'); tone('sawtooth', 520, 90, 0.1, 0.18); burst(0.55, 700, 0.9, 0.38); tone('sine', 85, 32, 0.28, 0.5, 0.08); burst(0.2, 2200, 0.25, 0.12, 'highpass'); }
    },
    // 充能步枪充能段：持续电光嗡鸣
    chargeZap() {
      tone('sawtooth', 880, 420, 0.95, 0.1);
      tone('square', 1400, 700, 0.9, 0.07);
      burst(0.12, 3200, 0.35, 0.9, 'highpass');
      tone('sine', 220, 90, 0.85, 0.12, 0.05);
    },
    // 充能步枪崩击：末段重击
    chargeBang() {
      burst(0.55, 280, 0.95, 0.42);
      tone('sawtooth', 160, 45, 0.28, 0.4);
      burst(0.22, 1800, 0.45, 0.12, 'highpass');
      tone('sine', 70, 28, 0.35, 0.5);
    },
